...
CAlgLegendre::Functor::Functor (int k, int start, double *mid):
    CAlgMidCoeff::Functor::Functor(k, start, mid)
{

}

double CAlgLegendre::Functor::operator() (int i, double x) {
...
void CAlgLegendre::ExtractWM(double *data, double *mid, int Nmid, int size)
{
    ...
    for (int i=0; i<cn; i++) {
        ...
        for (int k=0; k<8; k++) {
            f.setK(k);
            double res = CIntegrator::Integrate(-1, cstep, cc, f, intMethod);
            res *= ((2 * Lc[k] + 1) / 2.0);
            if (res <= this->a) {
                data[i*8 + k]--;
            }
...
void CAlgLegendre::ExtractExtWM(double *data, double *mid, int Nmid, int size)
{
    ...
    for (int i=0; i<cn; i++) {
        ...
        for (int k=0; k<8; k++) {
            f.setK(k);
            res[i * 8 + k] = CIntegrator::Integrate(-1, cstep, cc, f, intMethod);
