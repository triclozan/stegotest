...
CAlgConvolution::CAlgConvolution() {
    a = 0.35;
    gamma = 1;
    mode = 3;
    ch = 3;
...
void CAlgConvolution::Hide(QImage& container, QByteArray& watermark, QByteArray& key)
{
    qDebug() << a << " " << gamma;
    qDebug() << "L: " << L;
...
void CAlgConvolution::Restore(QImage& container, QByteArray& watermark, QByteArray& key)
{
    keyStruct keyData = *(keyStruct*)(key.constData());
    qsrand(keyData.seed);
    //qDebug() << keyData.seed;
...
void CAlgConvolution::Restore(QImage& container, QVector<double> &watermark, QByteArray& key)
{
    keyStruct keyData = *(keyStruct*)(key.constData());
    qsrand(keyData.seed);
    //qDebug() << keyData.seed;
...
void CAlgConvolution::GenKey(QByteArray& data)
{
    keyStruct keyData;
...
void CAlgConvolution::SetParams(QString& params)
{
    CParamHelper ph;
